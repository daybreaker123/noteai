...
const tutorComponents: Components = {
  h1: ({ node: _n, className, ...props }) => (
    <h1
      className={cn("mt-4 mb-2 text-lg font-semibold tracking-tight text-[var(--text)] first:mt-0", className)}
      {...props}
    />
  ),
  h2: ({ node: _n, className, ...props }) => (
    <h2
      className={cn(
        "mt-4 mb-2 border-b border-[var(--border)] pb-1.5 text-base font-semibold text-[var(--text)] first:mt-0",
        className
      )}
      {...props}
    />
  ...
  h3: ({ node: _n, className, ...props }) => (
    <h3 className={cn("mt-3 mb-1.5 text-sm font-semibold text-[var(--text)] first:mt-0", className)} {...props} />
  ),
  p: ({ node: _n, className, ...props }) => (
    <p className={cn("my-2 text-[0.9375rem] leading-relaxed text-[var(--text)] last:mb-0", className)} {...props} />
  ),
  strong: ({ node: _n, className, ...props }) => (
    <strong className={cn("font-semibold text-[var(--text)]", className)} {...props} />
  ),
  em: ({ node: _n, className, ...props }) => (
    <em className={cn("italic text-[var(--text)]", className)} {...props} />
  ),
  ...
  ol: ({ node: _n, className, ...props }) => (
    <ol
      className={cn("my-2 list-decimal space-y-1.5 pl-5 text-[0.9375rem] text-[var(--text)] marker:text-[var(--muted)]", className)}
      {...props}
    />
  ),
  ...
  hr: ({ node: _n, className, ...props }) => (
    <hr className={cn("my-4 border-0 border-t border-[var(--border)]", className)} {...props} />
  ),
  ...
  pre: ({ node: _n, className, children, ...props }) => (
    <pre
      className={cn("my-2 overflow-x-auto rounded-lg border border-[var(--border)] bg-[var(--chrome-50)] p-3", className)}
      {...props}
    >
      {children}
    </pre>
  ...
  table: ({ node: _n, className, ...props }) => (
    <div className="my-2 overflow-x-auto">
      <table className={cn("w-full border-collapse text-left text-sm text-[var(--text)]", className)} {...props} />
    </div>
  ),
  ...
  th: ({ node: _n, className, ...props }) => (
    <th className={cn("px-2 py-1.5 font-semibold text-[var(--text)]", className)} {...props} />
  ),
      ...
      img: ({ node: _n, className: imgClass, src, alt, ...props }) => {
        ...
        return (
          ...
          <img
            ...
            className={cn(
              "my-2 max-h-64 max-w-full rounded-lg border border-[var(--border)] object-contain",
